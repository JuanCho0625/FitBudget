...
export const getSavingGoals = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const result = goals.map(g => ({ 
      ...g.toObject(), 
     progressPercentage: g.targetAmount > 0 ? Math.min((g.currentAmount / g.targetAmount) * 100, 100).toFixed(1) : "0.0"
    }));
    res.json(result);
  } catch (error) { 
...
export const getSavingGoalById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.json({ 
      ...goal.toObject(), 
     progressPercentage: goal.targetAmount > 0 ? Math.min((goal.currentAmount / goal.targetAmount) * 100, 100).toFixed(1): "0.0"
    });
  } catch (error) { 
...
export const createSavingGoal = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // ── Emisión usando el objeto de la aplicación de Express ──
    const io = req.app.get('io');
    io.to(req.userId?.toString()).emit("update-saving-goal", {
      ...goal.toObject(),
...
export const updateSavingGoal = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const justCompleted = !wasCompleted && goal.status !== 'paused' && goal.currentAmount >= goal.targetAmount;
  ...
  const progress = (goal.currentAmount / goal.targetAmount) * 100;
    ...
    // 🔍 COMPROBACIÓN 1: Imprime el io para verificar que no esté "undefined"
    const io = req.app.get('io');
    ...
    // ── Enviar notificación en tiempo real si se completó ──
    if (justCompleted) {
...
export const deleteSavingGoal = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // ── Emisión usando el objeto de la aplicación de Express ──
    const io = req.app.get('io');
    io.to(req.userId?.toString()).emit("delete-saving-goal", req.params.id);
